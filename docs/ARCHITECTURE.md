...
- Long-distance jumps (≥ 4 MB remaining, e.g. opening near EOF of a multi-GB file) count newlines across disjoint byte chunks in parallel via `DispatchQueue.concurrentPerform`, then merge the per-chunk newline offsets into `entries` sequentially (line numbers are a prefix sum over newline counts, so the merge can't parallelize). The scan is capped at a bounded lookahead past the target line's estimated byte position (doubling and retrying if the estimate undershot) rather than always scanning to the literal end of the file, so a single long-distance jump doesn't grow `entries` to the file's full line count when only a nearby window is needed. If what's left past the lookahead window is itself within one threshold, the scan extends to the literal end anyway, so the file's true last line is never left without a `reachedEOF` determination
...
| 2026-06-16 | Parallel scan span is capped at a bounded, doubling lookahead instead of always scanning to `fileSize` | A 2026-06-16 code review flagged unbounded `entries` growth and missing cancellation on a single long-distance jump. The first fix attempt stopped the merge as soon as the target line was found but left `scanOffset` at the target's own offset instead of the full scanned span, so calling `ensureLineIndexed` once per line (as the cross-check test did) made every subsequent call re-trigger a near-identical multi-megabyte parallel scan — quadratic, observed taking minutes on a 100K-line file. The corrected design always advances `scanOffset` to the actual end of whatever span was scanned (bounded by the lookahead, not the target), and extends to the literal end of file whenever the remaining tail is itself small, so the file's true last line is never discovered without also setting `reachedEOF`. |
